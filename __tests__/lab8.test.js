describe('Basic user flow for Website', () => {
    ...
    beforeAll(async() => {
        ...
        //https://cse110-f2021.github.io/Lab8_Website previous URL
    });
    ...
    it('Clicking the "Add to Cart" button should change button text', async() => {
        ...
        const shadowRoot = await productItem[0].getProperty('shadowRoot');
        const productButton = await shadowRoot.$('button');
        await productButton.click();

        const innerText = await productButton.getProperty('innerText');
    ...
    it('Checking number of items in cart on screen after reload', async() => {
        ...
        for (let i = 0; i < 20; i++) {
            ...
            const buttonInnerText = await productButton.getProperty('innerText');

            if (buttonInnerText['_remoteObject'].value != "Remove from Cart")
                check = false;

        }
        const cartInnerText = await cartCount.getProperty('innerText');
        if (cartInnerText['_remoteObject'].value != 20)
            check = false;
    ...
    it('Checking number of items in cart on screen after reload', async() => {
        ...
        for (let i = 0; i < 20; i++) {
            ...
            if (buttonInnerText['_remoteObject'].value != "Add to Cart")
                check = false;

        }
        const cartInnerText = await cartCount.getProperty('innerText');
        if (cartInnerText['_remoteObject'].value != 0)
            check = false;
